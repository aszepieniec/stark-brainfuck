...
class IOTable(Table):
    ...
    def transition_constraints_ext(self, challenges):
        ...
        polynomials += [evaluation * iota + input_ - evaluation_next]
    ...
    def boundary_constraints_ext(self, challenges):
        ...
        return [x[IOTable.evaluation] - zero]  # evaluation
    ...
    def terminal_constraints_ext(self, challenges, terminals):
        ...
        field = challenges[0].field
        iota = challenges[self.terminal_index]
        iota_mpoly = MPolynomial.constant(iota)
        ...
        actual_terminal = evaluation_terminal * \
            MPolynomial.constant(iota ^ (self.height - self.length))

        print("in IOTable -- actual terminal:", actual_terminal, type(actual_terminal), "but evaluation terminal:",
        ...
        return [x[IOTable.evaluation] * iota_mpoly + x[IOTable.column] - actual_terminal]

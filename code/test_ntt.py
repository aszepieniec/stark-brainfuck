...
def test_multiply():
    ...
    for trial in range(20):
        ...
        lhs = Polynomial([field.sample(os.urandom(17))
                         for i in range(lhs_degree+1)])
        rhs = Polynomial([field.sample(os.urandom(17))
                         for i in range(rhs_degree+1)])

        fast_product = fast_multiply(lhs, rhs, primitive_root, n)
        slow_product = lhs * rhs

        assert(fast_product ==
               slow_product), "fast product does not equal slow product"
...
def test_batch_inverse():
    n = 100
    field = BaseField.main()
    array = [field.sample(os.urandom(8)) for i in range(n)]
    inverses = batch_inverse(array)
    assert(all((i*a) == field.one() for i, a in zip(inverses, array)))

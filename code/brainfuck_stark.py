...
class BrainfuckStark:
    ...
    def __init__(self, running_time, program, input_symbols, output_symbols):
        ...
        self.memory_table = MemoryTable(
            self.field, running_time, self.num_randomizers, smooth_generator, order)
        self.input_table = IOTable(
            self.field, len(input_symbols), smooth_generator, order)
        self.output_table = IOTable(self.field, len(
            output_symbols), smooth_generator, order)
    ...
    def prove(self, running_time, program, processor_matrix, instruction_matrix, input_matrix, output_matrix, proof_stream=None):
        ...
        extension_polynomials = reduce(
            lambda x, y: x+y, [table.interpolate_extension(omega, order) for table in extension_tables], [])
        ...
        quotient_codewords.concatenate(processor_extension.all_quotients_labeled(self.fri.domain, processor_extension.codewords, challenges=[a, b, c, d, e, f, alpha, beta, gamma, delta], terminals=[
        ...
        quotient_codewords.concatenate(instruction_extension.all_quotients_labeled(self.fri.domain, instruction_extension.codewords, challenges=[a, b, c, alpha, eta], terminals=[
            processor_instruction_permutation_terminal, instruction_evaluation_terminal]))
        # print("memory table:")
        quotient_codewords.concatenate(memory_extension.all_quotients_labeled(self.fri.domain, memory_extension.codewords, challenges=[
            d, e, f, beta], terminals=[processor_memory_permutation_terminal]))
        # print("input table:")
        quotient_codewords.concatenate(input_extension.all_quotients_labeled(self.fri.domain, input_extension.codewords,
                                                                             challenges=[gamma], terminals=[processor_input_evaluation_terminal]))
        # print("output table:")
        quotient_codewords.concatenate(output_extension.all_quotients_labeled(self.fri.domain, output_extension.codewords,
    ...
    def verify(self, proof, proof_stream=None):
        ...
        for index in indices:
            ...
            for constraint, bound in zip(processor_extension.transition_constraints_ext(challenges), processor_extension.transition_quotient_degree_bounds(challenges)):
                ...
                quotient = eval * self.xfield.lift(self.fri.domain(index) - processor_extension.omicron.inverse()) / (
                    self.xfield.lift(self.fri.domain(index) ^ BrainfuckStark.roundup_npo2(processor_extension.height)) - self.xfield.one())
                terms += [quotient]
                shift = self.max_degree - bound
                print("verifier shift:", shift)
                terms += [quotient *
            ...
            for constraint, bound in zip(instruction_extension.transition_constraints_ext(challenges), instruction_extension.transition_quotient_degree_bounds(challenges)):
                ...
                quotient = eval * self.xfield.lift(self.fri.domain(index) - instruction_extension.omicron.inverse()) / (
                    self.xfield.lift(self.fri.domain(index) ^ BrainfuckStark.roundup_npo2(instruction_extension.height)) - self.xfield.one())
                terms += [quotient]
                shift = self.max_degree - bound
                print("verifier shift:", shift)
                terms += [quotient *
            ...
            # transition
            unit_distance = memory_extension.unit_distance(
                self.fri.domain.length)
            next_index = (index + unit_distance) % self.fri.domain.length
            ...
            for constraint, bound in zip(memory_extension.transition_constraints_ext(challenges), memory_extension.transition_quotient_degree_bounds(challenges)):
                ...
                quotient = eval * self.xfield.lift(self.fri.domain(index) - memory_extension.omicron.inverse()) / (
                    self.xfield.lift(self.fri.domain(index) ^ BrainfuckStark.roundup_npo2(memory_extension.height)) - self.xfield.one())
                terms += [quotient]
                shift = self.max_degree - bound
                print("verifier shift:", shift)
                terms += [quotient *
            ...
            for constraint, bound in zip(input_extension.transition_constraints_ext(challenges), input_extension.transition_quotient_degree_bounds(challenges)):
                ...
                quotient = eval * self.xfield.lift(self.fri.domain(index) - input_extension.omicron.inverse()) / (
                    self.xfield.lift(self.fri.domain(index) ^ BrainfuckStark.roundup_npo2(input_extension.height)) - self.xfield.one())
                terms += [quotient]
                shift = self.max_degree - bound
                print("verifier shift:", shift)
                terms += [quotient *
            ...
            for constraint, bound in zip(output_extension.transition_constraints_ext(challenges), output_extension.transition_quotient_degree_bounds(challenges)):
                ...
                quotient = eval * self.xfield.lift(self.fri.domain(index) - output_extension.omicron.inverse()) / (
                    self.xfield.lift(self.fri.domain(index) ^ BrainfuckStark.roundup_npo2(output_extension.height)) - self.xfield.one())
                terms += [quotient]
                shift = self.max_degree - bound
                print("verifier shift:", shift)
                terms += [quotient *
